...
import { templateSchema as templateSchemaJson, nativeTypes } from "../template_schema";
...
export const TypeIdHelper = {
     ...
     */
    isPrimitiveType(in_typeid: string): boolean {
    ...
    */
    isTemplateTypeid(in_typeid): boolean {
     ...
     */
    isReservedType(in_typeid: string): boolean {
     ...
     */
    extractVersion(in_typeid): ExtractedVersion {
        if (!in_typeid) {
 ...
 */
    extractContext(in_typeid: string): ExtractedContext {
     ...
     */
    createSerializationTypeId(in_typeid: string, in_context: string, in_enum: boolean): string { // in_enum
     ...
     */
    sReferenceTypeId(in_typeid: string): boolean { // in_enum
     ...
     */
    extractReferenceTargetTypeIdFromReference(in_typeid: string): string { // in_enum
     ...
     * @return Is this a base template typeid?
     */
    isSchemaTypeid = function(in_typeid: string): boolean {
        return typeof in_typeid === 'string' && (in_typeid.indexOf(':') !== -1);
    },
     ...
     */
    extractTypeId(in_typeid): string {
     ...
     */
    nativeInheritsFrom(in_typeid: string, in_baseTypeid: string): boolean {
