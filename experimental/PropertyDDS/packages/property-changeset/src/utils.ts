 ...
 */
import _, { cloneDeep as deepCopy } from "lodash";
...
type PropertyContainerType = "array" | "map" | "set" | "root" | "NodeProperty";

const PRIMITIVE_TYPES = {};
TypeIdHelper.getPrimitiveTypeIds().map((typeid) => PRIMITIVE_TYPES[typeid] = true);
...
export class TraversalContext {
    _fullPath: string;
    _lastSegment: string | number;
    _lastSegmentString: string;
    _typeid: string|undefined;
    _splitTypeId: any;
    _userData: any;
    _traversalStopped: boolean;
    _nestedChangeSet: SerializedChangeSet;
    _parentNestedChangeSet: SerializedChangeSet;
    _propertyContainerType: PropertyContainerType;
    _arrayLocalIndex: any;
    _arrayOperationIndex: number|undefined;
    _arrayOperationOffset: number|undefined;
    _arrayIteratorOffset: number|undefined;
    _fullPostPath: string;
    _stackDepth: number;
    _typeStack: any[];
    _parentStack: any[];
    _containerStack: any[];
     ...
     */
    private getOperationType(): OperationType {
     ...
     */
    private getLastSegment(): string | number {
     ...
     * @return The last segment index
     */
    private getLastSegmentEscaped(): string | number {
     ...
     */
    private _isEmptyObject(in_context: { _nestedChangeSet: any; }) {
    ...
    private isLeafNode(): boolean {
        return PRIMITIVE_TYPES[this._typeid!] === true ||
     ...
     * @return The last segment index
     */
    private getPostLastSegment(): number | string {
     ...
     */
    private getTypeid(): string|undefined {
        return this._typeid;
    }
 ...
 */
private getNestedChangeSet(): SerializedChangeSet {
...
private replaceNestedChangeSet(in_newNestedChangeset: SerializedChangeSet) {
    ...
    if (this.getPropertyContainerType() === "template") {
        ...
        if (parent) {
            ...
            throw new Error(`TEMPORARY - INTERNAL: Was expected to replace a nested ChangeSet under "${this.getLastSegment()}" by "${JSON.stringify(in_newNestedChangeset)}", but could not find "${this.getTypeid()}" in "${JSON.stringify(this.getParentNestedChangeSet())}"`);
        }
 ...
 */
private getParentNestedChangeSet(): SerializedChangeSet {
