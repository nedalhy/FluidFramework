import { emptyField, FieldKinds, FieldSchema, LocalFieldKey,
         NamedTreeSchema,
         neverTree, StoredSchemaRepository,
...
export function convertPSetSchema(typeid: string, repository: StoredSchemaRepository) {
    ...
    for (const referencedTypeId of referencedTypeIDs.values()) {
        ...
        if (splitTypeId.context === "single") {
            ...
            const kind = splitTypeId.context === "array"
                ? FieldKinds.sequence.identifier : FieldKinds.optional.identifier;
        ...
        repository.updateTreeSchema(referencedTypeId, typeSchema);
    }
}
